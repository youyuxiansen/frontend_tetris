...
function canTetrominoMove(tetromino, rowOffset, colOffset) {
  for (let i = 0; i < tetromino.shape.length; i++) {
    for (let j = 0; j < tetromino.shape[i].length; j++) {
      if (tetromino.shape[i][j] !== 0) {
        let row = tetromino.row + i + rowOffset;
        let col = tetromino.col + j + colOffset;
...
function moveTetromino(direction) {
  ...
  if (!gameOver) {
    ...
    if (direction === "left") {
      ...
      if (canTetrominoMove(currentTetromino, 1, 0)) {
        added_new = true;
        // down
        eraseTetromino();
        row++;
...
function moveGhostTetromino() {
  ...
  while (canTetrominoMove(currentGhostTetromino, 1, 0)) {
...
function dropTetromino() {
  ...
  while (canTetrominoMove(currentTetromino, 1, 0)) {
    eraseTetromino();
    row++;
    currentTetromino.col = col;
    currentTetromino.row = row;
